...
export function RadioFilterGroup({ 
  ...
  return (
    <div className="flex items-center gap-3">
      <Label className="text-[16px] font-medium whitespace-nowrap">{label}</Label>
      <div className="flex items-center gap-2">
        {options.map((option) => (
          <button
            key={option.value}
            onClick={() => onChange(option.value)}
            className={`flex items-center justify-center gap-1.5 rounded-md px-3 h-[40px] text-[16px] font-medium transition-all ${
              value === option.value
